...
public class EditSendTo extends Activity {
    ...
    private class ButtonAddNewOnClickHandler implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            String emptyWarning = "You should enter a number!!! Please!";
            String fullWarning = "Too many numbers!!!!";
            String duplicateWarning = "You already have this one!!!";
            String successAdded = newNumber + " successfully added!";

            if(newNumber.isEmpty()){
                System.out.println(emptyWarning);
                Toast.makeText(EditSendTo.this, emptyWarning, Toast.LENGTH_SHORT).show();
            }
            else{

                String resultAdded;
                try{
                    resultAdded = messageData.addPhoneNumber(etPhone.getText().toString());
                }
                catch(SMSDataModelFullException excep){
                    Toast.makeText(EditSendTo.this, fullWarning, Toast.LENGTH_SHORT).show();
                    resultAdded = "FULL";
                ...
                if(resultAdded == "FULL"){
                    ...
                    if( resultAdded == "DUPLICATE"){
                    Toast.makeText(EditSendTo.this, duplicateWarning, Toast.LENGTH_SHORT).show();
                }
                else{
                    Toast.makeText(EditSendTo.this, successAdded, Toast.LENGTH_SHORT).show();
                    ((MyListViewAdapter) lvPhoneNumbers.getAdapter()).notifyDataSetChanged();
                }
    ...
    private class ButtonDeleteOnClickHandler implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            String successDelete = " Succesfully deleted!";

            rowView = (View) v.getParent();
            position = lvPhoneNumbers.getPositionForView(rowView);


            Toast.makeText(EditSendTo.this,messageData.getPhoneNumber(position) +  successDelete, Toast.LENGTH_SHORT).show();
            messageData.deleteNumber(position);

            selectedPosition = listViewItemSelectedHandler.getCurrentSelectedPosition();

            if(selectedPosition > position){
                selectedPosition -= 1;
            }

            listViewItemSelectedHandler.onItemSelected((ListView) rowView.getParent(), rowView, selectedPosition, selectedPosition);

            etPhone.setText(messageData.getPhoneNumber(selectedPosition));
            ...
            System.out.println(messageData.getNumPhoneNumbers());
