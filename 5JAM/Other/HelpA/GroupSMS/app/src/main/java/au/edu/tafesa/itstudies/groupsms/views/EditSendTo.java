...
public class EditSendTo extends Activity {
    ...
    private class ButtonUpdateOnClickHandler implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (newNumber.equals("")) {
                ...
                Toast.makeText(
                        EditSendTo.this,
                        "Update item " + (currentPositionInListView+1) + " which was " + oldNumber+" to "+ newNumber , Toast.LENGTH_SHORT).show();
    ...
    private class ButtonAddNewOnClickHandler implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (newNumber.equals("")) {
                ...
                try {
                    oldNumber = messageData.addPhoneNumber(newNumber);

                if(oldNumber.equals(SMSDataModelList.DUPLICATE))
                    {
                        Toast
                                .makeText(EditSendTo.this, "Duplicate Number", Toast.LENGTH_SHORT)
                                .show();
                    }

                    else if(oldNumber.equals(SMSDataModelList.FULL))
                    {
                        Toast
                                .makeText(EditSendTo.this, "Full Cannot add more Numbers to List", Toast.LENGTH_SHORT)
                                .show();
                    }
                    else {
                    Toast.makeText(
                            EditSendTo.this,
                            "Added " + (newNumber) , Toast.LENGTH_SHORT).show();
                    ((MyListViewAdapter) lvPhoneNumbers.getAdapter()).notifyDataSetChanged();
                }
    ...
    private class ButtonDeleteOnClickHandler implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            int position;
            int currentPositionInListView;
            oldNumber = etPhone.getText().toString();
            ...
            position = listViewItemSelectedHandler.getCurrentSelectedPosition();


            if(position >=  currentPositionInListView)
            {
               oldNumber = messageData.deleteNumber( currentPositionInListView);
                listViewItemSelectedHandler.onItemSelected((ListView) rowView.getParent(), rowView, position-1, currentPositionInListView);
                Toast.makeText(
                        EditSendTo.this,
                        "Deleted " + (currentPositionInListView +1) + " which was " + oldNumber, Toast.LENGTH_SHORT).show();
                ((MyListViewAdapter) lvPhoneNumbers.getAdapter()).notifyDataSetChanged();
            }
            else if(position <  currentPositionInListView)
            {
                oldNumber = messageData.deleteNumber( currentPositionInListView);
                listViewItemSelectedHandler.onItemSelected((ListView) rowView.getParent(), rowView, position, currentPositionInListView);
                Toast.makeText(
                        EditSendTo.this,
                        "Deleted " + (currentPositionInListView +1) + " which was " +  oldNumber, Toast.LENGTH_SHORT).show();
                ((MyListViewAdapter) lvPhoneNumbers.getAdapter()).notifyDataSetChanged();
